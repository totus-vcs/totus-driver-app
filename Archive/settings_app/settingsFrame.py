...
class SettingsFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.frontcam = customtkinter.CTkCheckBox(self, text="Front Camera On", command=self.frontcam_on_cmd)
        self.frontcam.grid(row=5, column=0, padx=10, pady=(10, 0), sticky="w")
        
        self.leftcam = customtkinter.CTkCheckBox(self, text="Left Camera On", command=self.leftcam_on_cmd)
        self.leftcam.grid(row=6, column=0, padx=10, pady=(10, 0), sticky="w")
        
        self.rightcam = customtkinter.CTkCheckBox(self, text="Right Camera On", command=self.rightcam_on_cmd)
        self.rightcam.grid(row=7, column=0, padx=10, pady=(10, 10), sticky="w")
    ...
    def frontcam_on_cmd(self): 
        self.frontcam.get()
        if self.frontcam.get() == 1: 
            api_requests.value_message("/frontcam/change_state", 'on')
        elif self.frontcam.get() == 0: 
            api_requests.value_message("/frontcam/change_state", 'off')
            
    def leftcam_on_cmd(self): 
        self.leftcam.get()
        if self.leftcam.get() == 1: 
            api_requests.value_message("/leftcam/change_state", 'on')
        elif self.leftcam.get() == 0: 
            api_requests.value_message("/leftcam/change_state", 'off')
            
    def rightcam_on_cmd(self): 
        self.rightcam.get()
        if self.rightcam.get() == 1: 
            api_requests.value_message("/rightcam/change_state", 'on')
        elif self.rightcam.get() == 0: 
            api_requests.value_message("/rightcam/change_state", 'off')